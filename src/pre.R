...
library(pacman)
p_load(interp, mboost, rpart, survival)
...
#' 
#' @export
pre_custom <- function(formula, data, family = gaussian, ad.alpha = NA, ad.penalty = "lambda.min",
  ...
  if (learnrate == 0) {
    ...
    fit_tree_return_rules <- function(formula, data, family = NULL, weights,
      ...
      if (tree.unbiased) {
        ...
        #fit rpart with only subset of features
        tree_predictors <- sample(colnames(data)[colnames(data) != "label"], mtry, replace = FALSE)
        
        fmla <- as.formula(paste("label ~", 
                                 paste(tree_predictors, collapse = " + ")))
        
        #environment(fmla) <- environment()
      
      
        tree <- rpart(formula = fmla, data = data[c(tree_predictors, "label")], control = NULL,
                      weights = weights)
    ...
    if (par.init) { # compute in parallel:
      rules <- foreach::foreach(i = 1:ntrees, .combine = "c", .packages = c("partykit", "pre", "rpart")) %dopar% {
