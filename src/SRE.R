...
cv.SRE <- function(inner_folds, tree_algorithm, RE_model_AUC, RE_model_Brier, RE_model_PG, GAM_recipe, metrics, train_bake, test_bake) {
  ...
  for(k in 1:nrow(inner_folds)) {
    ...
    if(tree_algorithm=="bagging") {
      ...
      RE_model_AUC_full <- train(XGB_recipe, data = train, method = "pre",
      ...
      RE_model_inner_Brier <- train(XGB_recipe, data = train, method = "pre",
                                 ...
                                 tuneGrid = getModelInfo("pre")[[1]]$grid( 
                                   maxdepth = (RE_model_AUC_full$results%>%slice_min(Brier)%>%dplyr::select(maxdepth))[[1]][1],
                                   ...
                                   sampfrac = 1,
                                   mtry = (RE_model_AUC_full$results%>%slice_min(Brier)%>%dplyr::select(mtry))[[1]][1],
                                   use.grad = FALSE),
      ...
      RE_model_inner_PG <- train(XGB_recipe, data = train, method = "pre",
                              ntrees = 100, family = "binomial", trControl = trainControl(method = "none", classProbs = TRUE),
                              tuneGrid = getModelInfo("pre")[[1]]$grid( 
                                maxdepth = (RE_model_AUC_full$results%>%slice_max(partialGini)%>%dplyr::select(maxdepth))[[1]][1],
                                penalty.par.val = c("lambda.min"), # λand γ combination yielding the sparsest solution within 1 standard error of the error criterion of the minimum is returned
                                sampfrac = 1,
                                mtry = (RE_model_AUC_full$results%>%slice_max(partialGini)%>%dplyr::select(mtry))[[1]][1],
      ...
      RE_model_inner_Brier <- train(XGB_recipe, data = analysis(inner_split$splits[[1]]), method = "pre",
                                       ntrees = 100, family = "binomial", trControl = trainControl(method = "none", classProbs = TRUE),
                                       tuneGrid = getModelInfo("pre")[[1]]$grid( 
                                         maxdepth = (RE_model_AUC_full$results%>%slice_min(Brier)%>%dplyr::select(maxdepth))[[1]][1],
                                         learnrate = (RE_model_AUC_full$results%>%slice_min(Brier)%>%dplyr::select(learnrate))[[1]][1],
                                         penalty.par.val = c("lambda.min"), # λand γ combination yielding the sparsest solution within 1 standard error of the error criterion of the minimum is returned
                                         sampfrac = 1,
                                         use.grad = TRUE), 
                                    tree.unbiased = FALSE, ad.alpha = 0, singleconditions = TRUE,
      ...
      RE_model_inner_PG <- train(XGB_recipe, data = analysis(inner_split$splits[[1]]), method = "pre",
                                    ntrees = 100, family = "binomial", trControl = trainControl(method = "none", classProbs = TRUE),
                                    tuneGrid = getModelInfo("pre")[[1]]$grid( 
                                      maxdepth = (RE_model_AUC_full$results%>%slice_max(partialGini)%>%dplyr::select(maxdepth))[[1]][1],
                                      learnrate = (RE_model_AUC_full$results%>%slice_max(partialGini)%>%dplyr::select(learnrate))[[1]][1],
    ...
    if(!is.null(RE_model_inner$finalModel$rules)) {
      SRE_train_rules_AUC <- fit_rules(inner_train_bake, drop_na(tibble(rules = RE_model_inner$finalModel$rules$description))$rules)
      SRE_test_rules_AUC <- fit_rules(inner_test_bake, drop_na(tibble(rules = RE_model_inner$finalModel$rules$description))$rules)
  ...
  return(list(best_AUC = final_SRE_fit_auc, best_Brier = final_SRE_fit_brier, best_PG = final_SRE_fit_pg))
}
